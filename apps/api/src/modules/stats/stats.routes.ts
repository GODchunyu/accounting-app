...
export function createStatsRouter(authService: AuthService, statsService: StatsService) {
  const router = Router();
  const requireAuth = createAuthMiddleware(authService);

  router.get("/stats/monthly", requireAuth, async (request: AuthenticatedRequest, response, next) => {
    try {
  ...
  router.get("/stats/categories", requireAuth, async (request: AuthenticatedRequest, response, next) => {
    try {
      const bookId = parseRequiredQuery(request.query.bookId, "bookId");
      const month = parseRequiredQuery(request.query.month, "month");
