...
export function createBillsRouter(authService: AuthService, billsService: BillsService) {
  const router = Router();
  const requireAuth = createAuthMiddleware(authService);

  router.get("/bills", requireAuth, async (request: AuthenticatedRequest, response, next) => {
    try {
  ...
  router.get("/bills/:id", requireAuth, async (request: AuthenticatedRequest, response, next) => {
    try {
      const bill = await billsService.getBill(request.currentUser!.id, request.params.id);
  ...
  router.post("/bills", requireAuth, async (request: AuthenticatedRequest, response, next) => {
    try {
  ...
  router.patch("/bills/:id", requireAuth, async (request: AuthenticatedRequest, response, next) => {
    try {
  ...
  router.delete("/bills/:id", requireAuth, async (request: AuthenticatedRequest, response, next) => {
    try {
      await billsService.deleteBill(request.currentUser!.id, request.params.id);
