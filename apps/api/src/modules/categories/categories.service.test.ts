...
describe("CategoriesService", () => {
  it("creates, updates, disables, reorders, and deletes unused categories for the current user", async () => {
    ...
    const disabled = await categoriesService.disableCategory(
      user.id,
      created.id,
    );
    ...
    await expect(
      categoriesService.deleteCategory(user.id, created.id),
  ...
  it("prevents users from mutating another user's categories", async () => {
    ...
    const [bobCategory] = await categoriesService.listCategories(
      bob.user.id,
      "expense",
    );
    ...
    await expect(
      categoriesService.disableCategory(alice.user.id, bobCategory!.id),
    ).rejects.toThrow("Category not found");
  });
});
